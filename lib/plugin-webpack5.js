...
class VueLoaderPlugin {
  apply(compiler) {
    ...
    for (const rawRule of rules) {
      ...
      // 遍历到匹配到 'foo.vue' ｜'foo.vue.html' 的规则后将规则赋值给 rawVueRule 并退出
    ...
    // 未获取到可以匹配到 'foo.vue' ｜'foo.vue.html' 的规则，直接报错
    if (!vueRules.length) {
      throw new Error(
    ...
    const refs = new Map()
    // 可以匹配到 'foo.vue' ｜'foo.vue.html' 的规则不需要 clone 进 clonedRules
    ...
    compiler.options.module.rules = [
      ...
      ...clonedRules,
      ...rules // 由于匹配 .vue 的vue-loader在 rules 中（在 clonedRules 中过滤掉了 rawVueRule），加上 rules 的执行顺序是从下到上，所以可以保证 rules中的rawVueRule执行顺序在 clonedRules 之前
    ]
  }
}
...
function match(rule, fakeFile) {
  ...
  // 返回可以匹配到 fakeFile 的规则，（由于 ruleSet 的参数为 [clonedRawRule]，因此 clonedRawRule 中 的 use 中有几个规则可以匹配 fakeFile，就返回几个规则 ）
  /* 例如下面的示例 匹配foo.vue 就可以匹配到 'vue-loader' 和 'match-vue-sfc-loader'
  {
    test: /\.vue$/,
    use: [
      {
...
function cloneRule(rawRule, refs, ruleCheck, ruleResource) {
  ...
  const res = {
    ...
    resourceQuery: (query) => {
      ...
      // 第一次经过 vue-loader 后request的query上才会加上 ?vue
      if (parsed.vue == null) {
        return false
      }
