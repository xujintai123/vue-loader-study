...
// Pitching Loader 中 remainingRequest的使用：https://webpack.docschina.org/api/loaders/#pitching-loader
module.exports.pitch = function (remainingRequest) {
  ...
  // 要插入一个行内 loader，这个loader 的作用是处理 scoped。这个loader接收的值应该是css，因此顺序应该是在css loader右侧的第一位。
  if (query.type === `style`) {
    const cssLoaderIndex = loaders.findIndex(isCSSLoader)
    if (cssLoaderIndex > -1) {
      // css loader及顺序在 css loader 左侧的 loader
      const afterLoaders = loaders.slice(0, cssLoaderIndex + 1)
      // 顺序在 css loader 右侧的 loader
      const beforeLoaders = loaders.slice(cssLoaderIndex + 1)
      const request = genRequest([
        ...afterLoaders,
        stylePostLoaderPath, // 插入在这里保证此loader接收的值是css代码
        ...beforeLoaders
      ])
      // console.log(request)
      return query.module
